...
export async function loadPersistedMetaLibraryAds(jobId: string): Promise<MetaLibraryAd[]> {
  ...
  return (data as Record<string, unknown>[]).map((row) => ({
    ...
    spend: (row.spend_json as MetaLibraryAd["spend"]) ?? null,
    impressions: (row.impressions_json as MetaLibraryAd["impressions"]) ?? null,
    audienceSize: (row.audience_size_json as MetaLibraryAd["audienceSize"]) ?? null,
    metaMetrics: (row.meta_metrics_json as MetaLibraryAd["metaMetrics"]) ?? null,
    metaDetailMetrics: (row.meta_detail_metrics_json as MetaLibraryAd["metaDetailMetrics"]) ?? null,
    ...
    finalMetrics: (row.final_metrics_json as MetaLibraryAd["finalMetrics"]) ?? null,
    currency: null,
